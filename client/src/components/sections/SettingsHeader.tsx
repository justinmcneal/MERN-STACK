...
const SettingsHeader: React.FC<SettingsHeaderProps> = ({
  ...
  return (
    <header className="bg-slate-900/60 backdrop-blur border-b border-slate-800/60 p-4 lg:p-6 z-50">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button
            onClick={onSidebarToggle}
            className="lg:hidden p-2 rounded-lg bg-slate-800/60 border border-slate-700/60 hover:bg-slate-700 transition"
          >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <button
              ...
              className="relative p-2 rounded-lg bg-slate-800/60 border border-slate-700/60 hover:bg-slate-700 transition"
            >
              <svg
            ...
            {notificationOpen &&
              createPortal(
                <div
                  className="fixed top-[70px] right-0 left-0 sm:right-[260px] sm:left-auto w-full sm:w-96 max-h-[70vh] bg-slate-900/95 backdrop-blur border border-slate-800/60 rounded-2xl shadow-xl z-[9999] overflow-hidden flex flex-col"
                >
                  {/* Header */}
          ...
          <div className="relative z-50">
            <div className="flex items-center gap-3 bg-slate-800/60 border border-slate-700/60 rounded-xl px-3 py-2 cursor-pointer z-50"
            ...
            {profileDropdownOpen &&
            createPortal(
              <div className="fixed top-[85px] right-[39px] w-44 bg-slate-900/95 backdrop-blur border border-slate-800/60 rounded-xl shadow-lg z-50">
                <button onClick={() => { navigate("/profile"); onProfileDropdownToggle(); }} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-slate-200 hover:bg-slate-800/50 transition-colors">
                  <User className="w-4 h-4 text-cyan-400" /> Profile
                </button>
                <button onClick={() => { handleLogout(); onProfileDropdownToggle(); }} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-slate-200 hover:bg-slate-800/50 transition-colors">
