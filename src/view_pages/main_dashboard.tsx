      ...
      <div className="relative z-10 flex h-screen">
        ...
        <div className={`${sidebarOpen?'translate-x-0':'-translate-x-full'} fixed inset-y-0 left-0 z-50 w-64 bg-slate-900/95 backdrop-blur border-r border-slate-800/50 transition-transform duration-300 lg:translate-x-0 lg:static lg:inset-0`}>
          <div className="flex items-center gap-3 p-6 border-b border-slate-800/50">
            ...
            <div className="font-bold text-lg"><span className="text-cyan-400">ArbiTrader</span><span className="text-purple-400 ml-1">Pro</span></div>
          </div>
          <div className="p-4">
            ...
            <nav className="space-y-2">
              {navigation.map(item=>(
                <button key={item.name} onClick={()=>setActiveTab(item.name)}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${activeTab===item.name?"bg-gradient-to-r from-cyan-500/20 to-purple-500/20 text-cyan-400 border border-cyan-500/30":"text-slate-400 hover:text-slate-300 hover:bg-slate-800/50"}`}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* Header */}
          <header className="bg-slate-900/50 backdrop-blur border-b border-slate-800/50 p-4 lg:p-6 relative z-50">
              ...
              <div className="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-2xl p-6">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-slate-700/50">
                        {["Token","From → To","Price Diff (%)","Gas Fee","Est Profit","Score"].map((h,i)=><th key={i} className="text-left text-xs text-slate-400 font-medium pb-3">{h}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {arbitrageOpportunities.map((a,i)=>(
                        <tr key={i} className="border-b border-slate-800/30">
                          <td className="py-3"><span className="font-medium text-slate-200">{a.token}</span></td>
                          <td className="py-3"><span className="text-slate-300 text-sm">{a.from} → {a.to}</span></td>
                          <td className="py-3 text-right"><span className="text-emerald-400 font-medium">{a.priceDiff}</span></td>
                          <td className="py-3 text-right text-slate-300">{a.gasFee}</td>
                          <td className="py-3 text-right text-cyan-400 font-medium">{a.estProfit}</td>
                          <td className="py-3 text-right">
                            <span className={`font-bold ${a.score>=90?'text-emerald-400':a.score>=80?'text-yellow-400':'text-slate-400'}`}>{a.score}</span>
                          </td>
                        </tr>
                      ))}
                    </tbody>
